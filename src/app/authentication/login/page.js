...
export default function LoginPage() {
    ...
    const handleSubmit = async (event) => {
      ...
      try {
        const response = await fetch("https://flask-production-75af.up.railway.app/medicos"); // Replace with verification endpoint if available
    ...
    <div className="w-screen bg-gradient-to-r from-[#006FEE] to-blue-900 h-screen flex items-center justify-center">
      ...
      <Modal isOpen={isOpen} onOpenChange={onOpenChange}>
        <ModalContent>
          {(onClose) => (
            <>
              <ModalHeader className="flex flex-col gap-1">Oops, tivemos um problema</ModalHeader>
              <ModalBody>
                <p> 
                  O médico não foi encontrado no sistema, ou a senha está incorreta. Tente novamente.
                </p>
              </ModalBody>
              <ModalFooter>
